...
class Sniffer:
	...
	def run_bufferScript(self,send,directory,interval,intf,ip):
		...
		send.cmd('python2 Framework/handler/buffer_script.py {} {} {} > {}.txt &'.format(interval, intf,ip,os.path.join(directory, intf+"_bufferQueue_{}".format(current_time))))

	def run_ssScript(self,send,directory,interval,ip):
		now = datetime.now()
		current_time = str(now.year) +"_" +str(now.month)+"_"+str(now.day)+"_"+str(now.hour)+"_" +str(now.minute)
		send.cmd('python2 Framework/handler/ss_script.py {} {} > {}.txt &'.format(interval,ip,os.path.join(directory, "_bbrValues_{}".format(current_time))))